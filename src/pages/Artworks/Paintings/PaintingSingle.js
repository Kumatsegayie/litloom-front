...
import { getPainting } from "../../../services/paintingsAPI";
...
const PaintingSingle = () => {
  ...
  const handleBack = () => navigateBackOr(navigate, "/paintings");
  ...
  // lock body scrolling and make page inert while fullscreen
  useEffect(() => {
    if (fullScreen) {
    ...
    return () => { document.body.style.overflow = ''; document.body.classList.remove('fullscreen-open'); };
  }, [fullScreen]);
  ...
  const handleDownload = async () => {
    const imagesArray = painting && painting.images ? (Array.isArray(painting.images) ? painting.images : [painting.images]) : [];
    const mediaUrl = imagesArray[galleryIndex] || painting.image;
    if (!mediaUrl) return;
    const file = sanitizeFilename(painting.title || "painting", "painting");
    try {
      await directDownload(mediaUrl, `${file}.jpg`);
    } catch (err) {
      console.error("Painting download failed", err);
  ...
  const imagesArray = painting && painting.images
    ? (Array.isArray(painting.images) ? painting.images : [painting.images])
    : [];
  ...
  const {
    ...
    contentId: painting?.id || id,
    contentTitle: painting?.title || "",
    contentSlug: painting?.slug || ""
  });
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getPainting(id)
      .then(p => { if (!mounted) return; setPainting(p); })
  ...
  if (error || !painting) {
    return (
      <section className="home-container">
        <PageErrorState
          title={error ? "Unable to load painting" : "Painting not found"}
          message={error ? buildConnectionErrorMessage("Paintings", error) : "This painting does not exist or is no longer available."}
  ...
  return (
    <section className="painting-single home-container">
      ...
      <div className="painting-single-layout">
        ...
        <div className="painting-full">
          {(() => {
            if (imagesArray.length > 0) {
              return (
                <div className="gallery-view" {...gallerySwipeHandlers}>
                  <img src={imagesArray[galleryIndex]} alt={`${painting.title} ${galleryIndex+1}`} />
                  {hasMultipleImages && (
            ...
            return <img src={painting.image} alt={painting.title} />;
          })()}
        ...
        <div className="painting-details">
          <p className="painting-author">{painting.artist}</p>

          <p className="painting-meta">{painting.category} • {painting.year}</p>

          <div className="painting-description">
            <div dangerouslySetInnerHTML={{ __html: formatRichTextToHtml(painting.description) }} />
          </div>

          {/* actions moved to floating FABs for a cleaner layout */}
        </div>
      </div>
        ...
        <div className="fullscreen-overlay" onClick={() => setFullScreen(false)}>
          <div className="fullscreen-modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="fullscreen-image-wrap" {...fullscreenSwipeHandlers}>
              {(() => {
                const src = imagesArray.length > 0 ? imagesArray[galleryIndex] : painting.image;
                return <img src={src} alt={painting.title} className="fullscreen-image" />;
              })()}
              {hasMultipleImages && (
      ...
      {!fullScreen && (
        <FixedBackButton onClick={handleBack} ariaLabel="Back to Paintings" />
      )}
      {!fullScreen && (
        <ContentActionDock
          actions={[
